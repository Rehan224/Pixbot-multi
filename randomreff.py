...
def request_otp(email, delay=60):
    try:
...
def save_successful_email(email):
    with open('email_terpakai.txt', 'a') as file:
        file.write(email + '\n')

def remove_email_from_data(email):
...
def main():
    ...
    # Meminta beberapa kode referral dan menyimpannya dalam daftar
    referral_codes = input("Masukan Kode Refferal (pisahkan dengan koma): ").split(',')
            ...
            if otp_body:
                ...
                if otp_code:
                    ...
                    if auth_data and 'access_token' in auth_data:
                        ...
                        # Menggunakan kode referral secara bergantian
                        referral_code = referral_codes[index % len(referral_codes)]
